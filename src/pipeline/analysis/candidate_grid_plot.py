...
from pipeline.common.paths import OUTPUTS_REPORTS_DIR

DEFAULT_CSV = OUTPUTS_REPORTS_DIR / "search_results_signal_pack3.csv"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot a distance vs chirp-mass candidate grid from a search-results CSV.")
    parser.add_argument("csv_path", nargs="?", type=Path, default=DEFAULT_CSV, help="Input CSV with columns real_mchirp, real_distance, candidate.")
    parser.add_argument("--output", type=Path, help="Output PNG path. Defaults to <csv_stem>_candidate_grid.png in the same folder.")
...
def load_grid(csv_path: Path) -> tuple[np.ndarray, list[float], list[float]]:
    ...
    for row in rows:
        if parse_candidate(row["candidate"]):
            grid[distance_index[float(row["real_distance"])], mchirp_index[float(row["real_mchirp"])]] = 1.0
