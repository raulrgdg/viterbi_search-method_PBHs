...
def get_optimal_tsft_list(min_nsigma_normed, mchirp_min=1e-4, mchirp_max=1e-1, n_mchirp=500, flow=61.1, psd_fhigh=4096.0, delta_f=0.25, fs_guess=128.0, n_tsft_candidates=300):
    ...
    if fs_opt < flow or fs_opt > psd_fhigh:
        raise ValueError(f"Optimized f*={fs_opt:.6f} Hz lies outside the PSD range [{flow:.6f}, {psd_fhigh:.6f}] Hz.")
    ...
    selected_idx = []
    constraints = LinearConstraint(valid_matrix.astype(float), lb=np.ones(n_mchirp), ub=np.full(n_mchirp, np.inf))
    bounds = Bounds(np.zeros(n_tsft_candidates), np.ones(n_tsft_candidates))
    integrality = np.ones(n_tsft_candidates, dtype=int)

    try:
        result = milp(c=np.ones(n_tsft_candidates), constraints=constraints, bounds=bounds, integrality=integrality)
