...
def build_mass_groups(
    ...
    while current_min < mchirp_max:
        ...
        if (
            shared_time_window(current_min, hi_fail, flow, fhigh, min_t_to_merger=min_t_to_merger) is not None
            and t_to_merger_from_mass_and_frequency(current_min, max_frequency_at_t_to_merger) >= min_t_to_merger
        ):
            hi_ok = hi_fail
        else:
            for _ in range(100):
                trial = geometric_midpoint(hi_ok, hi_fail)
                candidate_window = shared_time_window(
                    current_min,
                    trial,
                    flow,
                    fhigh,
                    min_t_to_merger=min_t_to_merger,
                )
                candidate_t_to_merger = max(
                    min_t_to_merger,
                    t_to_merger_from_mass_and_frequency(current_min, max_frequency_at_t_to_merger),
                )
        ...
        if window is None:
            break
        representative_t_to_merger = max(
            min_t_to_merger,
            t_to_merger_from_mass_and_frequency(current_min, max_frequency_at_t_to_merger),
        )
        ...
        if max_groups is not None and len(groups) >= max_groups and hi_ok < mchirp_max:
            ...
            if window is None:
                raise ValueError(
                    "Requested max_groups is too small for this mass range and frequency band."
                )
            representative_t_to_merger = max(
                min_t_to_merger,
                t_to_merger_from_mass_and_frequency(
                    groups[-1]["mchirp_min"], max_frequency_at_t_to_merger
                ),
            )
            if representative_t_to_merger > window[1]:
                raise ValueError(
                    "Requested max_groups is too small for the frequency-at-t_to_merger condition."
                )
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Return valid t_to_merger values for contiguous chirp-mass ranges "
            "inside a target GW-frequency band, requiring all masses in each "
            "range to be at or below a chosen frequency at that t_to_merger."
        )
    )
